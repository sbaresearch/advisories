...
untrusted certificates (e.g., self-signed), since the certificate
fingerprint is stored as SHA-1, although SHA-1 is considered weak and
should be replaced with SHA-256 or SHA-512.
...
As only the SHA-1 fingerprint of the certificate is stored and SHA-1 is
considered weak, an attacker could provoke a hash collision and thus perform
machine-in-the-middle attack on the TLS encrypted connection, resulting in a
total loss of confidentiality and integrity of the connection.

## Vulnerability Description ##

When permanently accepting an unknown TLS certificate for a specified
service, Cyberduck and Mountain Duck store the certificate fingerprint
to their own configuration file.
The certificate fingerprint is stored as SHA-1, which is considered weak.
...
![Security warning](./images/Security%20Warning.png)
  ...
  By clicking `No` the mounting process gets canceled,
  therefore to continue clicking `Yes` is required.

5. Now, the fingerprint of the certificate is stored at the configuration
...
The application stores the SHA-1 certificate fingerprint, although SHA-1 is
considered weak.
An attacker could provoke a hash collision and thus perform
machine-in-the-middle attack on the TLS encrypted connection, resulting in a
total loss of confidentiality and integrity of the connection.
...
application should store the certificate fingerprint using a modern hashing
...
* `2025-06-25` SBA Research assigned CVE-2025-41256
* `2025-06-25` Public disclosure via CVE

## References ##

* GitHub Security Advisory. Cyberduck and Mountain Duck - Weak Hash Algorithm for Certificate Fingerprint: <https://github.com/iterate-ch/cyberduck/security/advisories/GHSA-688c-vjrc-84rv>
